...
const static uint8_t ABV_P[] PROGMEM = {
...
const static uint8_t ABV_R[] PROGMEM = {
    /* by using PROGMEM the array stays in the program memory and is not copied to precious sram. */
    196, 181, 166, 158, 151, 145, 136, 129, 123, 117, 110, 104, 98, 92, 86, 79, 74, 68, 62, 56,
    50, 45, 39, 33, 27, 21, 15, 9, 3, 253, 247, 241, 235, 229, 224, 218, 212, 207, 201, 195,
    191, 188, 183, 178, 172, 165, 158, 151, 145, 139, 133, 127, 122, 116, 111, 105, 98, 93, 87, 80,
...
float TtoABV(float T, float P, bool nearAzeo)
{
    ...
    if (IndexABV < 0)
        return float(IndexABV) * 0.0625; // Below azeotrope
    if (IndexABV >= int16_t(sizeof ABV_R / sizeof *ABV_R))
        return 0; // Above 100 °C
    if (IndexABV < 29)
        return float(pgm_read_byte(&ABV_R[IndexABV]) + 768) / 10;
    if (IndexABV < 74)
        return float(pgm_read_byte(&ABV_R[IndexABV]) + 512) / 10;
    if (IndexABV < 138)
        return float(pgm_read_byte(&ABV_R[IndexABV]) + 256) / 10;
    return float(pgm_read_byte(&ABV_R[IndexABV])) / 10;
};
float TtoLiquidABV(int stillType, float T, float P)
{
    if (stillType == 1)
    {
        ...
        if (IndexABV < 0)
            return float(IndexABV) * 0.0625; // Below azeotrope
        if (IndexABV >= int16_t(sizeof ABV_P / sizeof *ABV_P))
            return 0; // Above 100 °C
        if (IndexABV < 29)
            return float(pgm_read_byte(&ABV_P[IndexABV]) + 768) / 10;
        if (IndexABV < 74)
            return float(pgm_read_byte(&ABV_P[IndexABV]) + 512) / 10;
        if (IndexABV < 138)
            return float(pgm_read_byte(&ABV_P[IndexABV]) + 256) / 10;
        return float(pgm_read_byte(&ABV_P[IndexABV])) / 10;
    }
    else
    {
...
float TtoVaporABV(int stillType, float T, float P)
{
    if (stillType == 1)
    {
        ...
        if (IndexABV < 0)
            return float(IndexABV) * 0.0625; // Below azeotrope
        if (IndexABV >= int16_t(sizeof ABV / sizeof *ABV))
            return 0; // Above 100 °C
        if (IndexABV < 114)
            return float(pgm_read_byte(&ABV[IndexABV]) + 768) / 10;
        if (IndexABV < 256)
            return float(pgm_read_byte(&ABV[IndexABV]) + 512) / 10;
        if (IndexABV < 314)
            return float(pgm_read_byte(&ABV[IndexABV]) + 256) / 10;
